package check_if_the_rectangle_corner_is_reachable_hard
...
func Test_check_if_the_rectangle_corner_is_reachable(t *testing.T) {
	tests := []struct {
		X, Y    int
		circles [][]int
	}{
		{X: 3, Y: 4, circles: [][]int{{2, 1, 1}}},
		{X: 3, Y: 3, circles: [][]int{{1, 1, 2}}},
		{X: 3, Y: 3, circles: [][]int{{2, 1, 1}, {1, 2, 1}}},
		{X: 3, Y: 3, circles: [][]int{{2, 4, 1}, {4, 4, 1}, {4, 2, 1}}},
	}
	for _, tt := range tests {
		t.Log(canReachCorner(tt.X, tt.Y, tt.circles))
	}
}
